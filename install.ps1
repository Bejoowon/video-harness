# 영상 하네스 세팅 스킬 설치 (Windows PowerShell)
#
...
try {
    ...
    if ($env:VIDEO_HARNESS_SOURCE) {
        $src = Join-Path $env:VIDEO_HARNESS_SOURCE "skills\$Skill"
    } else {
        ...
        $found = Get-ChildItem -Path $tmp -Directory | Where-Object { Test-Path (Join-Path $_.FullName "skills\$Skill\SKILL.md") } | Select-Object -First 1
        if (-not $found) { throw "받은 파일에서 스킬 폴더를 찾지 못했습니다." }
        $src = Join-Path $found.FullName "skills\$Skill"
    }
    if (-not (Test-Path (Join-Path $src "SKILL.md"))) { throw "스킬 폴더를 찾지 못했습니다: $src" }

    function Install-To([string]$Label, [string]$Root) {
        $skills = Join-Path $Root "skills"
        $dest = Join-Path $skills $Skill
    ...
    if ($wantClaude) { Install-To "Claude Code" (Join-Path $UserHome ".claude") }
    if ($wantCodex) { Install-To "Codex" (Join-Path $UserHome ".codex") }
} finally {
...
Write-Host "  3. 이렇게 말한다:  영상 하네스 세팅을 시작해줘"

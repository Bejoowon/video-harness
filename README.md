# 영상 하네스 (video-harness)

쇼츠·릴스·유튜브 영상을 AI 에이전트와 함께 만들기 위한 **스킬 두 개**입니다. Claude Code와 Codex에서 쓸 수 있고, macOS와 Windows를 지원합니다.

| 스킬 | 하는 일 | 이렇게 말하면 시작합니다 |
|---|---|---|
| `video-harness-setup` | 작업 공간을 인터뷰로 처음 세팅하고, 10초 영상을 실제로 렌더해서 세팅이 도는지 보여 줍니다 | `영상 하네스 세팅을 시작해줘` |
| `video-reference-finder` | 참고할 영상·채널을 찾아 링크가 진짜인지 다시 확인하고, 고른 것만 기록합니다 | `레퍼런스 찾아줘` |

설치 명령 하나로 두 스킬이 함께 들어갑니다. 에이전트가 쉬운 말로 한 번에 하나씩 물어보고, 고른 것만 설치합니다. 영상을 처음 만들어 보는 사람도 따라올 수 있게 만들었습니다.
...
세팅의 첫 질문은 "어떤 채널을 만들고 싶으세요?"입니다. 에이전트가 먼저 어떤 채널을 만들고 싶은지 대화로 파악하고, 참고할 채널과 주제 후보를 찾아 보여 드린 뒤에 세팅을 시작합니다. 혼자 알아서 세팅해 버리지 않습니다.
...
**직접 복사:** 이 저장소를 받아서 `skills/` 아래의 폴더들을 그대로 복사합니다. 두 스킬은 서로 없어도 각각 돌아가므로 하나만 복사해도 됩니다.

- Claude Code: `~/.claude/skills/` (개인용) 또는 프로젝트의 `.claude/skills/` (프로젝트용)
- Codex: `~/.codex/skills/`
...
닮고 싶은 영상·채널을 찾아 주는 두 번째 스킬입니다. 세팅 중에도, 세팅이 끝난 뒤 아무 때나 쓸 수 있습니다.

1. **짧게 물어봅니다** — 어떤 분야인지, 누가 보는지, 세로인지 가로인지, 어디서 찾을지(유튜브·인스타그램 릴스·틱톡), 그리고 **이번에 특히 보고 싶은 것**(자막 / 구성 / 컷 템포 / 말투 / 썸네일 / 제목)을 묻습니다. 이미 세팅된 작업 공간이면 채널에 적힌 방향을 읽어 와서 **비어 있는 것만** 묻습니다.
2. **찾아옵니다** — Aside CLI가 깔려 있으면 브라우저 에이전트에게 탐색을 맡기고, 없으면 에이전트가 직접 웹에서 찾습니다. 둘 다 안 되면 찾은 척하지 않고 고르는 기준을 알려 드린 뒤 링크를 받습니다.
3. **링크가 진짜인지 다시 확인합니다** — 에이전트가 가져온 목록은 주장일 뿐이라, 주소를 실제로 열어 봅니다. 없는 주소는 목록에서 빠지고, 제목·채널 이름은 실제 값으로 바뀝니다.
4. **고르는 것은 사용자입니다** — 표로 보여 드리고, 1~3개와 각각 마음에 드는 점을 받아 채널의 레퍼런스 목록에 `미분석`으로 기록합니다. 작업 공간이 아니면 지금 폴더에 `레퍼런스_후보.md`를 만듭니다.

보기만 합니다. 구독·좋아요·댓글·팔로우를 하지 않고, 로그인 정보를 입력하지 않고, 아무것도 내려받지 않습니다. 레퍼런스는 분석용이며 화면·소리를 가져다 쓰지 않습니다.

> **Aside CLI는 선택입니다.** 없어도 끝까지 돌아갑니다. 있으면 로그인된 브라우저로 더 넓게 찾을 수 있습니다.
...
- **레퍼런스 찾기**
  - *링크 재확인*: 유튜브 영상·쇼츠·채널과 틱톡 프로필을 실제 주소로 호출해서, 살아 있는 주소와 없는 주소가 갈리는 것을 확인했습니다.
  - *인스타그램*: **자동으로 확인할 수 없습니다.** 없는 주소에도 정상 응답을 주기 때문입니다. `직접 확인 필요`로 표시하고 직접 열어 보시라고 안내합니다.
  - *Aside 경로*: macOS에서 Aside CLI 1.26으로 **한 번 실제로 돌려 확인했습니다**(50대 대상 건강 세로 쇼츠, 유튜브·틱톡). 후보 8개를 받아 왔고 링크는 모두 실존하는 것으로 확인됐습니다. 다만 한 번 돌려 본 것이라 매번 이 정도 품질이 나온다고 보장하지는 못합니다. Aside가 없으면 이 경로 자체를 타지 않습니다.
...
video-harness/
├── skills/
│   ├── video-harness-setup/      세팅 스킬 (SKILL.md, scripts, references, assets)
│   └── video-reference-finder/   레퍼런스 찾기 스킬 (SKILL.md, scripts, references)
├── install.sh / install.ps1      한 줄 설치 (skills/ 아래를 전부 설치합니다)
└── tests/                        pytest 테스트
```

두 스킬은 서로를 import하지 않습니다. 레퍼런스 찾기가 작업 공간 설정에 값을 쓸 때만 세팅 스킬의 `config_tool.py`를 바깥에서 실행합니다 — 설정 파일을 쓰는 창구는 그것 하나여야 하기 때문입니다. 세팅 스킬이 없으면 레퍼런스 찾기는 단독 모드로 돕니다.

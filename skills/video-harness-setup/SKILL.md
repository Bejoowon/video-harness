...
**경로가 바뀌면 분기를 다시 한다.** 3부 1단계에서 작업 공간 경로를 확인하는데, 사용자가 지금 폴더가 아닌 다른 경로를 대면 그 경로로 `preflight.py`를 **다시 돌리고 위 표로 돌아가 분기부터 다시 정한다**. `config_tool.py init`은 그다음이다. 이미 설정 파일이 있는 폴더는 `init`이 아니라 8절 재실행 메뉴로 가야 하고, 기존 채널 폴더가 있는 곳은 7절 채택으로 가야 한다.
...
인터뷰는 세 부로 나뉜다. **1부 방향 파악 → 2부 조사 → 3부 세팅**이고, 순서를 바꾸지 않는다. 무엇을 만들고 싶은지 모르는 채로 세팅하면 남는 것은 빈 폴더뿐이다.

### 대화 규칙

이 규칙이 아래 모든 절보다 우선한다. 어긋나 보이는 문장이 있으면 여기를 따른다.

1. **질문을 하면 거기서 멈추고 사용자의 답을 기다린다.** 답이 오기 전에는 다음 질문으로도, 기록 명령으로도, 설치로도 넘어가지 않는다.
2. **추천(★)은 제안이다.** 에이전트가 스스로 골라서 기록하지 않는다. 기록 명령은 사용자가 그 질문에 답한 뒤에만 실행한다. 예외는 사용자가 직접 "알아서 해줘"·"모르겠어요"라고 답한 3부 4~9단계뿐이다(규칙 4).
3. **추측해서 채우지 않는다.** 사용자의 첫 말에 이미 들어 있는 정보(예: "건강 채널 만들자")는 다시 묻지 말고 되짚어 확인만 한다. 그 한마디를 근거로 나머지(시청자·범위·말투·포맷·모듈)를 대신 정하지 않는다.
4. **"알아서 해줘"·"추천대로"라고 해도 3부의 설치 단계(4~9)만 추천값으로 채운다.** 1부·2부와 10단계 요약 확인은 건너뛰지 않는다. 대신 그렇게 채운 값은 요약에서 "제가 고른 값"이라고 표시한다.
5. **사용자에게 물을 수 없는 환경(비대화형 실행)이면 세팅을 진행하지 않는다.** 물어야 할 질문 목록만 내놓고 멈춘다.

### 묻는 방법

- 한 번에 한 가지만 묻는다. 여러 질문을 한 번에 쏟지 않는다. 3부 표의 한 행이 한 질문이다. 3단계처럼 한 행에 값이 여러 개면 값마다 나눠 묻되, 기록은 행 단위로 한 번에 한다.
- 3부 질문에는 선택지를 모두 보여 주고, 추천 하나에 ★를 붙이고, 추천하는 이유를 한 줄 덧붙인다. 이유는 1부에서 들은 방향과 출발점에 이어 붙인다.
...
- 3부 4~9단계에서 "모르겠어요"·"아무거나"라고 하면 추천값으로 정하고 무엇으로 정했는지 한 줄로 알린다. **1부 질문에서는 그렇게 하지 않는다** — 대신 그 사람이 반응할 수 있는 구체적인 예를 두세 개 들어 주고 다시 묻는다(`discovery.md`).
- API 키·비밀번호 같은 비밀값은 채팅으로 받지 않는다. `.env.example`을 보고 본인이 `.env`에 직접 채우게 하고, 나중에 doctor가 "채워졌는지"만 확인한다.
- 채널에 들어갈 답(1부 전체 · 2부에서 고른 것 · 3부의 3 · 3-1 · 3-1a · 3-2)은 모았다가 `add-channel` 한 번으로 기록한다. 채널은 통째로 만들어야 해서다. 그 밖의 단계는 답마다 바로 기록한다.

### 1부 — 방향 파악

기술적인 질문보다 **먼저** 한다. 여기서 묻는 것은 취향이 아니라 사용자에 대한 사실이라 대신 골라 줄 수 없다 — **1부 질문에는 ★를 붙이지 않는다.** 한 번에 하나씩, 사람이 말하듯 묻는다.

1. **"어떤 채널을 만들고 싶으세요? 분야든 주제든 떠오르는 대로 말씀해 주세요."** → 한 줄 콘셉트(`concept`)의 재료다. 이미 말했으면("건강 채널 만들자") 다시 묻지 말고 "건강 쪽 채널을 만드시는 거군요"처럼 되짚어 확인만 한다.
2. **출발점** — 영상을 만드는 방식이 두 갈래로 갈라지고, 그 갈래가 3부의 추천을 바꾼다.
   ...
   → `workflow`. 헷갈려 하면 두 흐름을 각각 한두 문장으로 풀어 주고 다시 묻는다 — 찍은 영상 먼저는 찍어 둔 촬영본을 먼저 보고 쓸 구간을 골라 이야기를 만드는 방식이고, 대본 먼저는 할 말을 먼저 쓴 다음 거기에 맞는 화면을 찾거나 만드는 방식이다. 더 풀어서 설명할 말은 `workflows.md`에 있다.
3. **"누가 보면 좋겠어요? 나이대나 상황, 어떤 고민을 가진 분인지요."** → `audience`
4. **"그 분야에서 무엇을 다루고, 무엇은 다루지 않을 건가요?"** → `scope`
5. **"이 분야와 어떤 관계세요? 일로 하시는 전문가인지, 직접 겪어 보신 건지, 공부하면서 전해 주시는 건지요."** → `expertise`
6. **"어떤 느낌이면 좋겠어요? 말투와 분위기요. 얼굴이나 목소리는 나오나요?"** → `tone`
7. **"어디에, 얼마나 자주 올릴 건가요?"** → `platforms`
8. **"이미 좋아하거나 닮고 싶은 채널·영상이 있나요?"** 있으면 링크를 받고, 없으면 2부에서 찾아 주겠다고 말한다.

5번 답을 듣고, 틀린 말이 사람을 다치게 할 수 있는 분야(건강·의료·약·영양, 금융·투자, 법률, 안전)면 그 자리에서 알린다: "이 분야는 잘못된 정보가 사람을 다치게 할 수 있어서, 작업 공간에 엄격한 근거 규칙을 넣겠습니다." 그리고 `evidence: strict`로 기록한다. 그 밖에는 `normal`이고 따로 묻지 않는다.

여덟 가지를 다 들으면 **되짚는다** — 들은 것을 3~5줄로 정리해 보여 주고 "맞나요?"라고 묻는다. 맞다는 답을 받기 전에는 2부로 가지 않는다. 막연한 답을 푸는 법은 `discovery.md`에 있다.
...
먼저 묻는다: **"이 방향으로 레퍼런스와 주제를 찾아볼까요?"** 답을 받고 시작한다.

레퍼런스 찾기 스킬(`video-reference-finder`)이 설치돼 있으면 **레퍼런스는 그 스킬의 절차를 따라** 찾는다 — 조건을 짧게 묻고, 찾은 링크가 진짜인지 다시 확인하고, 사용자가 고른 것만 기록한다. 그 스킬이 없으면 아래 그대로 진행한다. 주제 조사는 어느 쪽이든 여기서 한다.

웹을 검색하거나 페이지를 열 수 있는 수단이 있을 때:

- **레퍼런스** — 같은 분야이면서 같은 포맷인 채널·영상 3~5개. 각각 링크 / 무엇인지 / 무엇을 배울 만한지(자막 · 구성 · 컷 템포 · 말투 · 색감 중 하나) / 주의할 점 하나.
- **주제** — 1부에서 말한 시청자가 실제로 묻는 주제 후보 8~10개. 각각 한 줄 근거와 어디서 봤는지(링크).
- 두 목록을 보여 주고 **사용자가 고르게 한다** — 레퍼런스 1~3개(각각 어디가 마음에 드는지도)와 먼저 만들 주제 3~5개.

정직하게 지킬 것:

- **실제로 열어 본 것만** 적는다. 채널 이름·URL·숫자를 지어내지 않는다.
- 구독자 수·조회수는 이번에 페이지에서 직접 본 경우에만 적고, 못 봤으면 아예 쓰지 않는다.
- `evidence: strict` 분야는 공공 보건기관·학회·진료지침처럼 공신력 있는 출처를 먼저 찾는다. 출처를 못 찾은 주제는 목록에서 빼거나 `출처 미확인`이라고 표시한다.

웹을 검색하거나 페이지를 열 수 있는 수단이 없을 때: 그렇다고 그대로 말한다. **찾아본 척하지 않는다.** 레퍼런스는 사용자에게 가져와 달라고 하고(아래 "레퍼런스 고르는 기준"), 주제는 사용자와 함께 떠올려 `조사하지 않은 아이디어`로 기록한다.

레퍼런스는 분석용이다. 레퍼런스 영상의 화면·소리를 내 영상에 가져다 쓰지 않는다. 진행 방법은 `discovery.md`에 더 있다.

### 3부 — 세팅

여기서부터 기술적인 질문이다. 3 · 3-1 · 3-1a · 3-2는 1부와 2부의 답에서 값을 **제안하고 확인을 받는다**. 왜 그 값인지 이유를 함께 말한다 — "쇼츠에 주 3회 올리신다고 하셔서 9:16·60초를 제안합니다. 괜찮으세요?" 4~9는 ★ 추천을 붙이되 이유를 1부의 방향과 출발점에 이어 붙인다.

#### 출발점에 따른 추천

4~9단계에서 어느 선택지에 ★를 붙일지 이 표를 보고 정한다. 선택지 자체는 그대로 다 보여 준다.
...
| 3 | 채널 이름, 포맷, 목표 길이, 언어, 고정 오프닝(선택) | 1부에서 들은 올리는 곳·주기에서 값을 제안하고 확인받는다. 포맷 세로 쇼츠 `9:16` ★ — 쇼츠·릴스 기본 / 가로 `16:9`. 길이 60초 ★. 한 줄 콘셉트는 1부 1번 답을 한 줄로 다듬어 제안한다 | (3-2까지 모아서 아래 `add-channel` 한 번) | — |
| 3-1 | 스타일 시작점 | 2부에서 레퍼런스를 골랐으면 레퍼런스에서 뽑기 `reference` ★ — 닮고 싶은 영상이 있으면 가장 빠르다(레퍼런스 분석 모듈 필요) / 기성 프리셋 `preset` / 직접 입력 `manual` / 나중에 정함 `later` | `add-channel`의 `--style-start reference`. `frame.md` 머리말에 `style_start: reference`로 남는다 | `module-watch.md`, `style-system.md` |
| 3-1a | (3-1이 `reference`일 때) 2부에서 고른 레퍼런스와 각각 마음에 드는 점을 그대로 확인받는다 | 2부에서 고른 것 ★ — 더 있으면 지금 받고, 하나도 없으면 "나중에 가져오기"로 넘긴다 | `add-channel`의 `--reference "URL::자막이 크고 가운데에 겹친다"`(여러 번). 채널의 `레퍼런스_목록.md`에 "미분석"으로 들어간다 | `module-watch.md` |
...
| 9 | 작업 규칙 | 버전은 `vN-then-final` ★(작업 중에는 _v2, _v3로 올리고 확정되면 최종본만 남긴다) 또는 `keep-all`. 샘플 길이 12초 ★. 대본 운율은 `meter` ★ — 읽을 때 리듬이 살아 쇼츠 내레이션이 귀에 잘 붙는다 / `free`(운율 규칙 없이 자연스럽게). 셋 다 물어보고, 사용자가 "그대로 좋다"고 답하면 그때 기본값으로 기록한다. 이 값은 앞으로의 작업 규칙으로 기록만 된다. 세팅 단계에서는 어떤 파일도 지우지 않는다. 실제 정리는 2단계에서 사용자 승인을 받고 한다 | `config_tool.py set "<ws>" rules.versioning '"vN-then-final"'` 그리고 `config_tool.py set "<ws>" rules.sample_seconds 12` 그리고 `config_tool.py set "<ws>" rules.script_rhythm '"meter"'` | `script-rhythm.md` |
| 10 | 요약을 보여 주고 확인 | — | 기록 없음. 3절로 간다 | — |

채널 기록(1부 답 · 2부에서 고른 것 · 3 · 3-1 · 3-1a · 3-2를 한 번에). 묻지 않은 항목의 옵션은 아예 빼고 실행한다:
...
python3 "${SKILL_DIR}/scripts/config_tool.py" add-channel "<ws>" \
  ...
  --audience "30~40대 동네 주민" --scope "가게 소개는 하고 광고성 리뷰는 하지 않는다" \
  --expertise "직접 다녀 본 사람" --tone "차분하고 다정한 말투, 얼굴은 안 나온다" \
  --platforms "유튜브 쇼츠·인스타 릴스, 주 3회" --evidence normal \
...
`--topic`은 2부에서 실제로 찾아본 주제고, `--topic-idea`는 조사 없이 함께 떠올린 아이디어다. 둘 다 채널의 `02_기획과자막/주제_후보.md`에 `조사함`·`아이디어`로 구분되어 들어간다. `--evidence strict`를 주면 `채널기준.md`의 "근거 규칙" 절이 엄격한 판으로 바뀐다.
...
출발점(1부 2번)의 답은 채널의 `채널기준.md`에 들어간다 — 머리의 "출발점" 한 줄과 "작업 순서" 절이 그 답에서 만들어진다. 출발점을 고르지 않은 채 채널을 만들면 작업 순서가 공통 순서로 남고, 나중 점검에서 `주의`로 뜬다.

1부 3~7번의 답은 같은 파일의 "방향" 절로, 근거 규칙은 "근거 규칙" 절로 들어간다. 아직 묻지 않은 항목은 `(아직 정하지 않음)`으로 남고, 시청자·범위가 둘 다 비면 점검에서 `주의`로 뜬다.
...
- **9 (대본 운율)** — 용어를 설명하지 말고 예를 하나만 보여 준다. "에이전트가 내레이션을 쓸 때 읽으면 리듬이 느껴지게 끊어 씁니다 — `오늘은 / 우리 동네 / 숨은 맛집 / 가 볼게`처럼요. 글자 수를 세어 보여 주는 도구도 함께 들어갑니다." 기본값이지만 묻기는 한다 — "이대로 갈까요?"까지 묻고, 좋다는 답을 받은 뒤에 기록한다. "운율은 필요 없다"고 하면 `free`로 기록한다. 이 규칙은 에이전트가 새로 쓰는 글에만 적용되고, 사람이 실제로 한 말을 받아 적은 전사 결과는 고치지 않는다는 점도 한 줄로 알린다.

#### 레퍼런스 고르는 기준 (2부·3-1a)

사용자가 직접 레퍼런스를 가져와야 할 때 이 네 가지를 함께 보여 준다.
...
- 방향(1부): 분야·시청자·다루는 범위·말투·근거 규칙
- 2부에서 고른 레퍼런스와 먼저 만들 주제
...
사용자가 답하지 않아 에이전트가 채운 값에는 "제가 고른 값"이라고 표시한다. 그래야 사용자가 그 줄만 보고 바로잡을 수 있다.
...
이미 세팅된 작업 공간에 채널을 더할 때. 새 채널에도 **1부(방향 파악)와 2부(조사)를 그대로 한 번 더** 하고, 그다음 3부의 채널 질문(3 · 3-1 · 3-1a · 3-2)만 묻는다. 모듈·규칙은 묻지 않는다. 방향과 출발점은 채널마다 다를 수 있으므로 이미 있는 채널의 답을 물려받지 않는다.
...
채택 경로에서도 3부 1·2단계(작업 공간 이름, 에이전트)를 먼저 묻는다. 에이전트를 묻지 않고 넘어가면 기본값(`claude`)이 그대로 들어가, Codex만 쓰는 사람의 작업 공간에도 `CLAUDE.md`가 놓인다.
...
채택 결과에 등록된 채널 목록이 나온다. 방향과 출발점은 아예 비어 있고 포맷·유형·길이는 기본값(9:16 세로, 내레이션 쇼츠, 60초)으로 들어간다.

채널마다 1부 질문을 한 번씩 한다. 채널이 이미 있으므로 더 가볍게 — 출발점, 누가 보면 좋겠는지, 무엇을 다루고 무엇은 다루지 않는지를 묻고, 민감한 분야면 근거 규칙도 알린다. 나머지(말투·올리는 곳·이 분야와의 관계)는 사용자가 말해 주는 만큼만 받는다. 답은 채널이 이미 등록돼 있으므로 `add-channel`이 아니라 목록 번호를 짚어 `set`으로 기록한다. 포맷·길이도 맞는지 확인해 다르면 같은 방식으로 고친다.

```
python3 "${SKILL_DIR}/scripts/config_tool.py" set "<ws>" channels.0.workflow footage-first
python3 "${SKILL_DIR}/scripts/config_tool.py" set "<ws>" channels.0.audience "30~40대 동네 주민"
python3 "${SKILL_DIR}/scripts/config_tool.py" set "<ws>" channels.0.scope "가게 소개는 하고 광고성 리뷰는 하지 않는다"
python3 "${SKILL_DIR}/scripts/config_tool.py" set "<ws>" channels.0.evidence strict
...
`채널기준.md`는 이미 만들어졌으면 스캐폴드가 덮어쓰지 않는다. 채택한 채널은 아직 없으므로 이 값들을 먼저 기록하고 폴더 만들기로 가면 "방향" 절이 채워진 채로 생긴다.

그다음 3부의 4~9단계 중 **비어 있는 모듈만** 묻고, 4절의 설치와 5절의 스모크 테스트로 이어 간다. "비어 있는 모듈"은 설정값이 기본값 그대로인 모듈이다(렌더 엔진 `hyperframes`, 편집기 `none`, 목소리 `record`, 소재 `["own-footage"]`, Higgsfield 인증 `none`, 레퍼런스 분석 `false`). 9단계는 모듈이 아니라 작업 규칙이므로 항상 한 번 확인한다. 물어본 뒤에는 3절 요약·확인을 반드시 다시 거친다.

## 8. 재실행 메뉴

설정 파일이 이미 있으면 처음부터 묻지 않는다. 일곱 가지 중 하나를 고르게 한다.

| 고른 것 | 할 일 |
|---|---|
| 채널 추가 | 6절 |
| 채널 방향 다시 잡기 | 그 채널에 1부 질문을 다시 하고, 받은 답마다 `config_tool.py set "<ws>" channels.N.audience "…"`처럼 목록 번호로 기록한다(`audience` · `scope` · `expertise` · `tone` · `platforms` · `evidence`). 2부 조사도 다시 할지 함께 묻는다. `채널기준.md`는 이미 만들어진 파일이라 덮어쓰이지 않으므로, 그 파일의 "방향"·"근거 규칙" 절은 새 답에 맞게 손으로 고쳐 준다 |
| 채널 출발점 바꾸기 | 1부 2번 질문을 다시 하고 `config_tool.py set "<ws>" channels.N.workflow '"footage-first"'`로 덮어쓴다(`N`은 목록 번호). `채널기준.md`는 이미 만들어진 파일이라 스캐폴드가 덮어쓰지 않으므로, 그 파일의 "작업 순서" 절은 새 출발점에 맞게 손으로 고쳐 준다 |
...
| 인터뷰 이어서 하기 | `config_tool.py show "<ws>"`로 지금까지의 답을 읽는다. `channels`가 비었으면 1부(방향 파악)부터, 모듈 값이 기본값 그대로면 그 단계부터 다시 묻는다. 기본값을 의도적으로 고른 경우와 구분할 수 없으므로, 이어 가기 전에 지금 값을 한 화면으로 보여 주고 "이대로 둘지"를 먼저 확인한다 |
| 재점검 | `doctor.py "<ws>"`를 돌리고 표와 조치를 전한다 |
| 스모크 테스트 다시 | `smoke_test.py "<ws>"`(필요하면 `--channel`) |

"Higgsfield 추가" 같은 요청은 3부 7단계(소재 수급)만 다시 타면 된다. 모듈을 바꾼 뒤에는 `scaffold.py "<ws>"`를 한 번 더 돌려 새 모듈의 도구를 `도구/`에 배치한다. 멱등이라 기존 파일을 덮어쓰지 않으므로 다시 돌려도 안전하다.
...
| 문서 | 언제 읽나 |
|---|---|
| `discovery.md` | 1부 방향 파악과 2부 조사를 진행할 때. 막연한 답을 푸는 법, 조사 결과를 보여 주는 법, 웹을 쓸 수 없을 때 |
| `workflows.md` | 1부 2번 출발점을 묻거나, 사용자가 두 흐름 중 어느 쪽인지 헷갈려 할 때 |
...
| `module-watch.md` | 레퍼런스 분석 스킬을 설치하거나 2부·3-1a로 레퍼런스를 모을 때 |
